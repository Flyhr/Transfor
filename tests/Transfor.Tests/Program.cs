...
static void TestMediaAssetGridBackfill()
{
    RunSta(() =>
    {
        var grid = new MediaAssetGrid();
        var context = new MediaRequestContext(null, null);
        var unknown = new MediaVariant(new Uri("https://x/unknown.webp"), null, null, null, null, null, "image/webp", null, MediaVariantSource.StructuredData, context);
        var post = new ResolvedMediaPost(MediaProviderId.Douyin, new Uri("https://v.douyin.com/a/"), "1", "t", "a", new MediaAsset[]
        {
            new MediaAsset(0, MediaKind.Image, new MediaVariant[] { unknown }),
        });
