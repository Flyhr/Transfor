...
以用户确认的“媒体 / 解析成功并下载中”设计图为基准：冷白工作区、92px 原生窄侧栏、青绿流程轨，以及底部内嵌下载队列。该图只作为视觉方向，不新增业务字段或下载状态。

## 目标与边界

- 仅改 `src/Transfor/Features/ModernUi` 的 Web UI 和原生宿主侧栏；旧 WinForms 预览 UI 不变。
- 固定浅色 Focused Flow：主色 `#2DD4BF`，深操作色 `#0F766E`，辅助色 `#38BDF8`；不提供深色或主题切换入口。
...
- 所有 CSS token 使用 `--ff-*` 前缀；卡片圆角 14px、主控件高度 36px、内容间距 24px。
